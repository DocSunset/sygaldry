...
add_library(sygaldry-components INTERFACE)
    add_library(sygaldry-components-portable INTERFACE)
        # arduino-hack, alphabetically missing here, is added only where required by a platform
        add_subdirectory(components/portable/button)
        add_subdirectory(components/portable/debug_printer)
    target_link_libraries(sygaldry-components INTERFACE sygaldry-components-portable)
    if (ESP_PLATFORM)
    add_library(sygaldry-components-esp32 INTERFACE)
    add_library(sygaldry-components-arduino INTERFACE)
        add_subdirectory(components/portable/arduino-hack)
        add_subdirectory(components/esp32/arduino-hack)
        add_subdirectory(components/arduino/two_wire)
        add_subdirectory(components/arduino/trill_craft)
        add_subdirectory(components/esp32/button)
        add_subdirectory(components/esp32/gpio)
        #add_subdirectory(components/esp32/icm20948)
        add_subdirectory(components/esp32/trill)
    target_link_libraries(sygaldry-components INTERFACE sygaldry-components-esp32)
    target_link_libraries(sygaldry-components INTERFACE sygaldry-components-arduino)
    endif()
