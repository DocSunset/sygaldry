\page page-sygaldry-utility_components-portable-basic_logger Basic Logger
...
If we imagine we have an instantiation of this template where the putter `put`
has a string stream member from which we can read what we've printed, we can
test our implementation like this:
...
above, is used for tests and puts the printed messages into a string. We
define the putter and declare an instantiation of the basic logger template,
which we define in a separate `.cpp` file so that the instantiation will
only be compiled once.

```cpp
// @#'test_logger/sygaldry-utility_components-portable-test_logger.hpp'
/*
...
```cpp
// @#'test_logger/sygaldry-utility_components-portable-test_logger.test.cpp'
/*
Copyright 2023 Travis J. West, https://traviswest.ca, Input Devices and Music
Interaction Laboratory (IDMIL), Centre for Interdisciplinary Research in Music
Media and Technology (CIRMMT), McGill University, Montréal, Canada, and Univ.
...
```cmake
# @#'test_logger/CMakeLists.txt'
set(lib sygaldry-utility_components-portable-test_logger)
...
add_library(${lib} INTERFACE)
    ...
    target_link_libraries(${lib}
            INTERFACE sygaldry-utility_components-portable-basic_logger
            )
target_link_libraries(sygaldry-utility_components INTERFACE ${lib})
...
Another, useful for informal experimentation and bindings in environments where
the C library `putc` is available, puts messages onto the standard output. The
physical implementation layout is the same.

```cpp
// @#'cstdio_logger/sygaldry-utility_components-portable-cstdio_logger.hpp'
/*
Copyright 2023 Travis J. West, https://traviswest.ca, Input Devices and Music Interaction Laboratory
(IDMIL), Centre for Interdisciplinary Research in Music Media and Technology
(CIRMMT), McGill University, Montréal, Canada, and Univ. Lille, Inria, CNRS,
...
```cmake
# @#'cstdio_logger/CMakeLists.txt'
set(lib sygaldry-utility_components-portable-cstdio_logger)
...
```cpp
// @#'basic_logger/sygaldry-utility_components-portable-basic_logger.hpp'
#pragma once
/*
Copyright 2023 Travis J. West, https://traviswest.ca, Input Devices and Music
Interaction Laboratory (IDMIL), Centre for Interdisciplinary Research in Music
...
set(lib sygaldry-utility_components-portable-basic_logger)
